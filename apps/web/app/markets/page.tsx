...
export default function MarketsPage() {
  ...
  return (
    <div className="p-3 space-y-3">
      {/* Stats Header */}
      {stats && (
        <div className="flex items-center gap-4 bg-bg-card border border-border-primary rounded-xl px-4 py-2.5">
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-text-muted uppercase tracking-wide">24H Volume:</span>
            <span className="text-sm font-mono font-bold text-accent-green">${formatNumber(stats.totalVolume)}</span>
          </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center bg-bg-card border border-border-primary rounded-lg overflow-hidden">
          <span className="px-2.5 py-1.5 text-[11px] font-semibold text-accent-yellow bg-accent-yellow/10 border-r border-border-primary">
            Trending
          </span>
          {TIME_RANGES.map((t) => (
            <button
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-all ${
              filter === f.key
                ? "bg-bg-active border-accent-blue/40 text-text-primary shadow-glow"
      ...
      <div className="bg-bg-card border border-border-primary rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]">
            <thead>
              <tr className="text-[10px] uppercase tracking-wider text-text-muted border-b border-border-primary bg-bg-secondary/50">
            ...
            <tbody>
              {loading && list.length === 0 ? (
                ...
                list.map((t, idx) => {
                  ...
                  // Simulated 5m/1h changes based on 24h for display variety
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        <Link href={`/tokens/${t.id}`} className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-accent-blue/30 to-accent-purple/30 flex items-center justify-center text-[10px] font-bold text-text-primary border border-border-primary">
                            {t.ticker?.[0] ?? "?"}
                          </div>
                          <div>
