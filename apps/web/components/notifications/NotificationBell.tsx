  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative p-1.5 rounded-lg text-text-muted hover:text-text-primary hover:bg-bg-hover transition-colors"
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 bg-accent-red text-white text-[9px] rounded-full min-w-[14px] h-3.5 px-1 flex items-center justify-center font-bold">
            {unreadCount}
          </span>
        )}
      </button>
      {open && (
        <div className="absolute top-full mt-1 right-0 w-80 max-h-96 overflow-y-auto bg-bg-card border border-border-primary rounded-xl shadow-dropdown z-50 animate-fade-in">
          <div className="px-3 py-2.5 border-b border-border-primary flex items-center justify-between">
            ...
            {unreadCount > 0 && (
              <span className="text-[10px] text-accent-blue">{unreadCount} new</span>
            )}
          </div>
            ...
            notifications.map((n) => (
              ...
              >
                <div className="flex items-start gap-2">
                  {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-accent-blue mt-1.5 flex-shrink-0" />}
